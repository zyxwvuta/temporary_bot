...
load_dotenv('main.env')  # Ensure 'main.env' contains TOKEN and CHAT_ID
...
def main():
    ...
    for event in events:
        ...
        print(f"Checking event: {event_name}, Scheduled at: {event_hour}:{event_minute}, Current time: {current_hour}:{current_minute}")

        # Only proceed if it's time to send reminders
        if current_hour == event_hour and abs(current_minute - event_minute) <= 1:
            print(f"Attempting to send reminder for {event_name} at {event_time}.")
            ...
            if reminder_time == "afternoon" and remind_days < MAX_REMIND_DAYS:
                ...
                if success:
                    c.execute('UPDATE events SET remind_days = remind_days + 1 WHERE id = %s', (event_id,))
                    print(f"Sent morning reminder for {event_name}. Day {remind_days + 1}/{MAX_REMIND_DAYS}")
                    reminder_sent = True
                    break
